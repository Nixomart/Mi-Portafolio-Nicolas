export const data = {
  infotech: [
    {
      tags: ["angular", "docker", "expressjs", "bootstrap"],
      name: "Aplicacion web para gestion de un gimnasio",
      where: "Universitario",
      how: "Aplicacion web para la gestion de un gimnasio, cuenta con diferentes roles como Usuario, Entrenador y Admin",
      job: [
        "Jwt para la gestion de sesiones",
        "Dockerizacion de las aplicaciones",
        "Uso de Angular para la aplicacion Frontend",
        "Express js como Framework de node js para el servidor",
        "MongoDB como base de datos",
        "Se uso Vercel para el backend, MongoDb Atlas, Netlify",
        "Bootstrap",
      ],
    },
    {
      tags: [
        "nextjs",
        "springboot",
        "mysql",
        "tailwind",
        "mercadopago",
        "cicd",
        "typescript",
      ],
      name: "Aplicacion web de votos",
      where: "Universitario / Encargado del frontend y de CICD",
      how: "Projecto final en el que se puede se hizo una aplicacion web para la votacion del proximo candidato a presidente",
      job: [
        "Jwt para la gestion de sesiones",
        "Dockerizacion de las aplicaciones",
        "Uso de Nextjs para la aplicacion Frontend",
        "Uso de los hooks useState y useEffect ",
        "SpringBoot (POO, Pipeline, Builder, DTO)",
        "MYSQL como base de datos",
        "CICD usando github Actions para la aplicacion Frontend",
        "Uso de TailwindCSS para los estilos de la web",
        "Uso de la API mercado pago para la compra de puntos",
      ],
    },
    {
      tags: ["reactjs", "expressjs", "mercadopago", "tailwind", "typescript"],
      name: "Ver Catalogo Online",
      where: "Personal",
      how: "Projecto personal basado en la idea de que las tiendas pequeñas paguen una subscripcion minima para tener su tienda virtual",
      job: [
        "Firebase como Backend ",
        "Libreria react router dom para el ruteo de la aplicacion",
        "useContext para la mantension de la informacion rapida",
        "Componentes Reutilizables",
        "Uso de los hooks useState y useEffect ",
        "Material Tailwind",
        "Uso de la API de mercado pago para la subscripciones",
        "Express js como API para la gestion de subscripcion",
        "Uso de TailwindCSS para los estilos de la web",
      ],
    },
    {
      tags: [
        "reactjs",
        "expressjs",
        "mercadopago",
        "tailwind",
        "Docker",
        "cicd",
        "typescript",
      ],
      name: "Pedir Turno Online",
      where: "Personal",
      how: "Projecto que es una aplicacion web que tiene lo nesesario para la gestion de una clinica, gestion de turnos, pacientes, especialistas y demas",
      job: [
        "Firebase como Backend ",
        "Libreria react router dom para el ruteo de la aplicacion",
        "useContext para la mantension de la informacion rapida",
        "Componentes Reutilizables",
        "Uso de los hooks useState y useEffect ",
        "Material Tailwind",
        "Uso de la API de mercado pago para la subscripciones",
        "Express js como API para la gestion de subscripcion",
        "Uso de TailwindCSS para los estilos de la web",
        "Librerias como Dayjs, React Calendar",
        "Uso de servidor VPS para la escucha de turnos y envios por Whatssap",
        "Dockerizacion de App",
        "CICD",
      ],
    },
    {
      tags: ["c#", "wpf", "sqlserver"],
      name: "Sistema gestion de estacionamientos",
      where: "Universitario",
      how: "Projecto universitario grupal en el que se uso WPF (Windows Presentation Foundation) para la creacion del sistema",
      job: [
        "C# como lenguaje de programacion",
        "SQL server como base de datos",
        "Stored Procedures para las consultas complejas",
        "Deployment de Aplicacion creando archivo instalador",
      ],
    },
    {
      tags: ["c#", "windowsforms", "sqlserver"],
      name: "Sistema gestion de optica",
      where: "Universitario",
      how: "Projecto universitario grupal en el que se uso Windows Forms para la creacion del sistema",
      job: [
        "C# como lenguaje de programacion",
        "SQL server como base de datos",
        "Stored Procedures para las consultas complejas",
        "Deployment de Aplicacion creando archivo instalador",
      ],
    },
    {
      tags: ["tailwind", "reactnative", "typescript"],
      name: "Aplicaion IOS / Android sobre nutricion",
      where: "Personal",
      how: "Aplicacion que se ha seguido en base a un Curso de React Native",
      job: [
        "Typescript",
        "NativeWind para el estilo de la app",
        "Uso de Hooks (useState, useEffect, useContext)",
      ],
    },
  ],
  frontend: [
    {
      name: "reactjs",
      image: "https://www.ritechpune.com/backend/courseicon/ReactJS.png",
    },
    {
      name: "typescript",

      image:
        "https://i.pinimg.com/564x/66/d5/23/66d5238900aab3d7b86dc5e53a77c817.jpg",
    },
    {
      name: "docker",
      image:
        "https://i0.wp.com/thinkground.studio/wp-content/uploads/2019/04/190421_Docker-Logo.png?resize=314%2C268",
    },
    {
      name: "tailwind",
      image:
        "https://yt3.googleusercontent.com/ikv41jMTr1uHGdILrJhvbfVJcDt4oqhwApKX37TjAleF_cRPbF2W-waj7uMnS5JySvnlvAlTCg=s900-c-k-c0x00ffffff-no-rj",
    },
    {
      name: "angular",
      image:
        "data:image/png;base64,",
    },
    {
      name: "bootstrap",

      image:
        "https://blog.artegrafico.net/lab/js/login-carousel/assets/img/bootstrap.png",
    },
  ],
  backend: [
    {
      name: "typescript",
      image:
        "https://i.pinimg.com/564x/66/d5/23/66d5238900aab3d7b86dc5e53a77c817.jpg",
    },
    {
      name: "docker",
      image:
        "https://i0.wp.com/thinkground.studio/wp-content/uploads/2019/04/190421_Docker-Logo.png?resize=314%2C268",
    },
    {
      name: "firebase",
      image:
        "https://www.gstatic.com/devrel-devsite/prod/v670576875a5cc9e38d1ef4dc0b1c46d2dc586275242dba27f9cea38af0674100/firebase/images/touchicon-180.png",
    },
    {
      name: "express",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTXgAuAwfMMDkwZScsLkBAeZ4GjyfB7GtIzcA&usqp=CAU",
    },
    {
      name: "mercadopago",
      image:
        "https://yt3.googleusercontent.com/h6cGMuMQo9kjtFACrBi5X86evZz8oudwh1Wo5gjIinW6UqwdnSTXH0Yj_D8gS-QlQ7OHLGnfDQ=s900-c-k-c0x00ffffff-no-rj",
    },
    {
      name: "springboot",
      image:
        "https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTm1iBtf8Doeac9meBtxvGb6oGqOuEhU2z6YPvdB1RsiEUGAeFnrtVjB-SzcBvowceMwSQ&usqp=CAU",
    },
    {
      name: "postgress",
      image:
        "https://upload.wikimedia.org/wikipedia/commons/thumb/2/29/Postgresql_elephant.svg/1200px-Postgresql_elephant.svg.png",
    },
    {
      name: "mysql",
      image: "https://cdn-icons-png.flaticon.com/512/5968/5968313.png",
    },
  ],

  learning: [
    {
      name: "docker",
      image: 
    "https://i0.wp.com/thinkground.studio/wp-content/uploads/2019/04/190421_Docker-Logo.png?resize=314%2C268",

    },
    {
      name: "nextjs",
      image: 
    "https://seeklogo.com/images/N/next-js-icon-logo-EE302D5DBD-seeklogo.com.png",

    },
    {
      name: "reactnative" ,
      image: 
    "https://solidstudio.io/_next/image?url=https%3A%2F%2Fcdn.sanity.io%2Fimages%2Flofvu8al%2Fproduction%2Faabfcf1734f9e1e4379f4f80042bdf11e2e57dbf-420x308.png&w=1080&q=75",

    },
    {
      name: "wpf",
      image: 
    "https://scientiait.blob.core.windows.net/programandoamedianoche/wp-content/uploads/2009/09/develop-wpf-and-xaml-programs.jpg",

    },
    {
      name:"windowsforms" ,
      image: 
    "https://i.pinimg.com/474x/45/1c/3e/451c3e4fab9236c8166ba774b715bd2c.jpg",

    }
    ,{
      name: "cicd",
      image:
    "https://s3.amazonaws.com/vipm-io-media-files-production/media/package-list-images/7d7747a7-68b1-4cd7-9527-19eb96be9d7c.png",

    }
  ],
  texts: [
    {
      title: "Douvery - 2022",
      link: "https://www.douvery.com/",
      how: [
        "Maquetaciones Responsivas (grid - flexbox)",
        "Creación de Componentes Reutilizables (Operadores Ternarios)",
        "Uso de Librerias (React Icons - Tailwind - React Bootstrap - Formik)",
        "Creación de nuevas funcionalidades (useEffect - useContext - useState - useMemo)",
      ],
      paragraph:
        'Fue mi primer trabajo "Formal" en donde tenia que hacer entregas de avances en el tiempo estimado y documentar lo que hacia',
    },
    {
      title: "Agencia de Marketing - Perú - 2022",
      how: [
        "euphoria.pe",
        "thelittlemarketperu.com",
        "santos-films.com",
        "lacorrientedelgolfo.net",
      ],
      paragraph:
        "Estuve como Diseñador haciendo uso de la Herramienta figma - Creación de Logos, Banners, imágenes publicitarias - Rediseños de Webs en Wordpresss/React js - Estrategias de Marketing / Facebook ADS / Segmentación ",
    },
    {
      title: "Desarrollador Web Freelancer - 2021 - Actualidad",
      how: [
        "piedrasuma.com",
        "gato.tv",
        "mandarinatv.com",
        "mohinorafm.com",
        "jrcontenidos.com.ar",
      ],
      paragraph: "Actualmente hago paginas web usando el CMS wordpress. ",
    },
  ],
};
export default data;
